...
Preuploaded barcodes are allocated to sales based on the `seat_ordinal` property provided when setting the barcodes against the listing. Barcodes are allocated from the lowest available `seat_ordinal` up. If you want to force Stubhub to allocate high to low, reverse the `seat_ordinal` order vs the seats - see examples below.
